...
def main():
    ...
    if escolha.isdigit() and 1 <= int(escolha) <= len(arquivos):#VAI VER SE O NUMEOR DIGITADO É UM ARQUIVO DISPONIVEL
        ...
        checkpoint = torch.load(nome_arquivo)#LOAD NO ARQUIVO QUE É USADO EM REDE NEURAL 
    ...
    else:#SE NAO FOR UM ARQUIVO EXISTENTE ELE vai criar um novo modelo
        ...
        juncao, amostras, ruido, tamanho_rede, epocas_treino, lr = menu_configuracao()#VAI PARA AS CONFIG DO OUTRO ARQUIVO PY
        min_x = min(t[0] for t in juncao)
        max_x = max(t[1] for t in juncao)
